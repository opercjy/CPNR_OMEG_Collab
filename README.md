...
# CPNR-OMEG-colab: ⁶⁰Co 선원 기반 액체섬광체 검출기 Geant4 전산모사

## 1. 개요 (Overview)

본 프로젝트는 학문 후속 세대용 ⁶⁰Co (코발트-60) 감마선 교정 선원을 이용한 액체섬광체(LS) 검출 시스템의 응답 특성을 평가하기 위한 Geant4 시뮬레이션입니다. 이 프로젝트는 Geant4의 고급 기능을 활용하여 두 가지 주요 방식으로 데이터를 출력하도록 설계되었습니다. 
프로그램 목적상 명세에 광학/광전증배관 물리 모델은 생략되어 있습니다. **CPNR_OMEG_colab_low_energy_optical** 프로젝트에서 2개 PMT, 광학/광전증배관 물리 모형이 되었습니다.
